...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_TIMER:
        ...
        if (ev == EVENT_LONG_PRESS)
        {
            ...
            sprintf(buf,
                "[EVENT]  btn=%d  action=%s  state=%s",
                i + 1,
                btn_event_str(ev),
                btn_state_str(g_fsm[i].state));

            AppendLog(buf);

            SendMessage(g_hstatus,
                SB_SETTEXTA,
                0,
                (LPARAM)"  LONG PRESS detected");
...
static void OnBtnRelease(int idx)
{
    ...
    sprintf(buf,
        "[EVENT]  btn=%d  action=%s  held_ms=%lu  state=%s",
        idx + 1,
        btn_event_str(ev),
        (unsigned long)held,
        btn_state_str(g_fsm[idx].state));
